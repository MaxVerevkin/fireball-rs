...
//
//         let flash_distance = (flash - solution.flash).norm();
//         let vel_angle = vel
//             .normalize()
//             .dot(&solution.velocity.normalize())
//             .min(1.0)
//             .acos();
//         let vel_diff = (vel.norm() - solution.velocity.norm()).abs();
//
//         if flash_distance > 200. || vel_angle > 1f64.to_radians() || vel_diff > vel.norm() * 0.03 {
//             eprintln!("==== FAILED TESTCASE ==== ");
//             eprintln!("Flash error = {}", flash_distance);
//             eprintln!("Velocity angle = {}", vel_angle.to_degrees());
//             eprintln!("Velocity rel error = {}", vel_diff / vel.norm());
//         } else {
//             done += 1;
//         }
//     }
...
//         let vel_diff = (vel.norm() - solution.velocity.norm()).abs();
//
//         if flash_distance > 5_000. || vel_angle > 1f64.to_radians() || vel_diff > vel.norm() * 0.03
//         {
//             eprintln!("==== FAILED TESTCASE ==== ");
//             eprintln!("Flash error = {}", flash_distance);
//             eprintln!("Velocity angle = {}", vel_angle.to_degrees());
...
//         let vel_diff = (vel.norm() - solution.velocity.norm()).abs();
//
//         if flash_distance > 5_000. || vel_angle > 1f64.to_radians() || vel_diff > vel.norm() * 0.05
//         {
//             eprintln!("==== FAILED TESTCASE ==== ");
//             eprintln!("Flash error = {}", flash_distance);
//             eprintln!("Velocity angle = {}", vel_angle.to_degrees());
//             eprintln!("Velocity rel error = {:.1}%", vel_diff / vel.norm() * 100.0);
//         } else {
//             done += 1;
//         }
//     }
...
//
//     let (flash, vel) = gen_rand_traj(mean_lat, mean_lon, &mut rng);
//     let data = gen_data(mean_lat, mean_lon, flash, vel, 100, 0, None, None, &mut rng);
//
//     let mut solver = Solver::new(
//         data,
//         Params {
...
//
//     for i in 0..n_total {
//         let duration = rng.gen_range(2f64..7f64);
...
//             let k_start_global = UnitVec3::new_normalize(p1 - global_pos);
//             let k_end_global = UnitVec3::new_normalize(p2 - global_pos);
//             let mut k_start: Azimuthal = k_start_global.to_local(geo_pos).into_inner().into();
//             let mut k_end: Azimuthal = k_end_global.to_local(geo_pos).into_inner().into();
//
...
//                 global_pos,
//                 (k_start_global.into_inner() + k_end_global.into_inner()).normalize(),
//                 // k_end_global.into_inner(),
//                 vel,
//             );
...
//     let file_path = format!("generated_data/{file_id}.toml");
//     eprintln!("Generated data written to {file_path}");
//     let mut file_out = File::create(file_path).unwrap();
//     std::io::Write::write_all(&mut file_out, data_str.as_bytes()).unwrap();
//
...
//     let dir = (flash - Vec3::from(p1)).normalize() * rng.gen_range(10_000.0..30_000.0);
//
//     (flash, dir)
// }
