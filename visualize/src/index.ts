...
interface Stage {
  ...
  traj: any, // Line
  sigmas: any, // Option<Sigmas>
}

function to_degrees(a: number): number {
...
function vec3_scal_mul(a: Vec3, num: number): Vec3 {
  return { x: a.x * num, y: a.y * num, z: a.z * num }
}

function vec3_norm_sq(a: Vec3): number {
  return a.x * a.x + a.y * a.y + a.z * a.z
}

function vec3_norm(a: Vec3): number {
...
function vec3_angle_degrees(a: Vec3, b: Vec3): number {
...
function extract_all_params(data: Db): string[] {
  const all_params: string[] = []
  for (const event of Object.values(data.events))
    for (const param of Object.keys(event.runs))
      if (!all_params.includes(param))
        all_params.push(param)
  return all_params
}

function extract_all_stages(data: Db, params: string): [string, number][] {
  let stage_names: [string, number][] = []
  for (const [_, event] of Object.entries(data.events)) {
    let counts: Record<string, number> = {}
    let run_stages: [string, number][] = []
    for (const stage of event.runs[params]) {
      const count = counts[stage.title] ?? 0
      run_stages.push([stage.title, count])
...
function process_data(data: Db, params_: string) {
  ...
  const events = Object.entries(data.events)
    .filter(a => params in a[1].runs)
    .sort((a, b) => b[1].observers_count - a[1].observers_count)
  ...
  for (const [stage_name, stage_i] of stage_names) {
    ...
    ["Distance, km", "Angle, °", "El. Angle, °", "Info"].forEach((name, i) => {
      const cell = document.createElement("th")
  ...
  for (const [event_name, event] of events) {
    ...
    for (const [stage_name, stage_i] of stage_names) {
      ...
      if (event.answer.dir == null) {
        ...
        const el_angle_value = Math.abs(to_degrees(Math.asin(vec3_dot(
          vec3_cross(event.answer.dir, vec3_normalize(vec3_sub(event.avg_observer, event.answer.point))),
          run.traj.direction,
        ))))
