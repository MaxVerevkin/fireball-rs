...
void process_whitnesses(tinyxml2::XMLElement *, FILE *);
void process_placemark(tinyxml2::XMLElement *, int, FILE *);
...
int main(int argc, char **argv) {
    // Check args
    if (argc < 2) {
        std::cerr << "Usage: kml file1 [file2 ...]\n";
        return 1;
    }

    // Open output file
    FILE *outfile = fopen("kml.toml", "w+");
...
void process_whitnesses(tinyxml2::XMLElement *pRoot, FILE *outfile) {
    ...
    while (pRoot) {
        ...
        tinyxml2::XMLElement *pPlacemark =
            pRoot->FirstChildElement("Placemark");
        while (pPlacemark) {
...
void process_placemark(tinyxml2::XMLElement *pPlacemark, int experience,
                       FILE *outfile) {
    // Get basic info
    ...
    double lon, lat;
    sscanf(pPlacemark->FirstChildElement("Point")
    ...
    auto push_json = [&](const char *label, double value) {
        if (value != -1) fprintf(outfile, "%s = %lf\n", label, value);
    };

    // Write to file
    fprintf(outfile, "[[sample]]\n");
    fprintf(outfile, "lat = %lf\n", lat);
    fprintf(outfile, "lon = %lf\n", lon);
